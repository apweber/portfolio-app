...
function salaryScore(target: number | null, min: number | null, max: number | null): number {
...
function remoteScore(user: WorkPref | null, job: WorkPref | null): number {
...
function locationScore(user: string | null, job: string | null): number {
  if (user === null || job === null) return 100;
...
export function calculateFitScore(input: FitInput): FitResult {
  const { weights } = input;
  const weightSum =
    weights.skillsWeight + weights.salaryWeight + weights.remoteWeight + weights.locationWeight;
  if (weightSum !== 100) throw new Error(`Weights must sum to 100, got ${weightSum}`);
